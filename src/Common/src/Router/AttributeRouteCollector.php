...
class AttributeRouteCollector implements AttributeRouteProviderInterface
{
    ...
    {
        // PSR-4 ile uyumlu şekilde namespace'e dönüştür
        $basePath = APP_ROOT.'/src/';
        $relative = str_replace([$basePath, '/', '.php'], ['', '\\', ''], $filePath);

        // Sınıf adını al
        $className = $relative; // örnek: SomeModule\src\Handler\FindAllByPagingHandler

        // Eğer `src` dizini namespace'e dahil değilse onu da temizle
